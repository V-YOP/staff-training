...
import { Either, Left } from '@/monads';
...
export function resultSequence<T, E>(resArr: Result<T, E>[]): Result<T[], E> {
  function helper(acc: T[], subResArr: Result<T, E>[]): Result<T[], E> {
    if (subResArr.length === 0) return Ok(acc)
    const [head, ...tail] = subResArr
    return head.andThen(value => helper([...acc, value], tail))
  }
  return helper([], resArr)
}